...
class ChargesController < ApplicationController
  ...
  def create
    ...
    flash[:notice] = "Thanks for the payment, #{current_user.name}! You can now create and edit private wikis."  
  ...
  def new
    @stripe_btn_data = {
      ...
      # key: 'pk_test_M2lgcfgcrqSqJg1xFDibtiui',
      description: "Premium Membership - #{current_user.name}",
      amount: 15_00
    }
    puts "------------------------------------------------------------------------"
    p Rails.configuration.stripe[:publishable_key]
  end
